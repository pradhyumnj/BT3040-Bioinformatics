...
def getFasta(name, kind = 'id'):
    """Retrive sequences in FASTA format from Uniprot"""
    import requests as r
    base = "http://www.uniprot.org/uniprot/"
    Id = name
    current = base + Id + ".fasta"
    seq = r.get(current)
    return seq.text.strip('\n')

def readClustal(fileName):
    import re
    with open(fileName) as file:
        lines = file.read()
        seqs = re.findall('sp\|.*',lines)
    ...
    for sq in seqs:
        ...
        if x in ids:
            ...
            ids[x] = y
    return list(zip(*list(ids.items())))
